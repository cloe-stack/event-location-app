...
app.get("/make-server-12d1bb49/health", (c) => {
  return c.json({ status: "ok" });
});

// Debug endpoint to check environment variables
app.get("/make-server-12d1bb49/debug", (c) => {
...
app.get("/make-server-12d1bb49/dev/users", async (c) => {
  try {
    const supabase = createClient(
      Deno.env.get('SUPABASE_URL') ?? '',
      Deno.env.get('SUPABASE_SERVICE_ROLE_KEY') ?? '',
...
app.delete("/make-server-12d1bb49/dev/users/:email", async (c) => {
  try {
...
app.get("/make-server-12d1bb49/test", (c) => {
  return c.json({ message: "Server is working!", timestamp: new Date().toISOString() });
});

// Sign up endpoint
app.post("/make-server-12d1bb49/signup", async (c) => {
  try {
...
app.get("/make-server-12d1bb49/locations", async (c) => {
  try {
...
app.get("/make-server-12d1bb49/locations/:id", async (c) => {
  try {
...
app.post("/make-server-12d1bb49/locations", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
...
app.put("/make-server-12d1bb49/locations/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
...
app.delete("/make-server-12d1bb49/locations/:id", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
...
app.post("/make-server-12d1bb49/upload-image", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
...
app.delete("/make-server-12d1bb49/images/:path", async (c) => {
  try {
    const accessToken = c.req.header('Authorization')?.split(' ')[1];
